require("http").createServer((_, res) => res.end("Uptime Kuro")).listen(8080)
...
require('./Kuro.js')
nocache('../Kuro.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./index.js')
...
console.log(color(`🚩 YT CHANNEL: Kuro`,'magenta'))
console.log(color(`🚩 GITHUB: Kuro-Zann `,'magenta'))
...
console.log(color(figlet.textSync(`Kuro`, {
font: 'Standard',
...
require('./Kuro')(XeonBotInc, m, chatUpdate, store)
} catch (err) {
console.log(err)}})
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			if (res.announce == true) {
				...
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nInfo Group Hanya Bisa Di Edit Oleh Admin, Sekarang Hanya Admin Yang Dapat Mengedit Info Group`,
				});
			} else if (res.restrict == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\nInfo Group Bisa Di Edit Oleh Semua member, Sekarang Member Dapat Mengedit Info Group`,
				});
			} else if(!res.desc == ''){
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, { 
					text: `「 Group Settings Change 」\n\n*Deskripsi Group Telah Diubah Menjadi*\n\n${res.desc}`,
				});
      } else {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Group Settings Change 」\n\n*Nama Group Telah Diubah Menjadi*\n\n*${res.subject}*`,
				});
			} 
...
if (anu.action == 'add') {
XeonBotInc.sendMessage(anu.id, { image: {url:ppuser}, caption : `Halo @${num.split("@")[0]}, Selamat Datang Di ${metadata.subject} Semoga Betah`},{ quoted : {
key: {
fromMe: false, 
...
XeonBotInc.sendMessage(anu.id, { image: {url:ppuser}, caption : `Goodbye @${num.split("@")[0]} Jangan Balik Lagi Lu Ye`},{ quoted : {
key: {
fromMe: false, 
participant: `0@s.whatsapp.net`, 
...({ remoteJid: "status@broadcast" }) 
...
} else if (connection === "open") { XeonBotInc.sendMessage(owner + "@s.whatsapp.net", { text: `Kuro Botz Started` }); }
